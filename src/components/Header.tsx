...
export default function Header({ theme = 'light' }: HeaderProps) {
    ...
    return (
        <>
            <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${bgClass} ${isScrolled || isSearchOpen ? 'py-4' : 'py-6'}`}>
                <div className="px-6 relative h-12 flex items-center justify-between">
                    {/* Normal Header Content */}
                    <div className={`w-full flex items-center justify-between transition-opacity duration-200 ${isSearchOpen ? 'opacity-0' : 'opacity-100'}`}>
                        ...
                        <div className={`flex items-center gap-8 ${textColor}`}>
                            ...
                            <nav className="hidden md:flex gap-6 text-xs tracking-widest font-medium">
                                <Link href="/sales/new-york-ny" className="hover:opacity-80">BUY</Link>
                                <Link href="/rentals/new-york-ny" className="hover:opacity-80">RENT</Link>
                                <a href="#" className="hover:opacity-80">SELL</a>
                                <a href="#" className="hover:opacity-80">AGENTS</a>
                            </nav>
                        </div>
                        ...
                        <div className={`hidden md:flex gap-6 transition-opacity duration-300 ${textColor} text-xs tracking-widest font-medium`}>
                            <a href="#" className="hover:opacity-80">NEW DEVELOPMENT</a>
                            <a href="#" className="hover:opacity-80">WORLD OF ELLIMAN</a>
                        </div>

                        {/* Play/Pause Button */}
                        <div className={`fixed right-6 top-6 transition-opacity duration-300 ${isSearchOpen ? 'opacity-0 pointer-events-none' : (textColor === 'text-white' && isScrolled ? 'opacity-0 pointer-events-none' : textColor === 'text-[#181728]' ? 'opacity-0 pointer-events-none' : 'opacity-100')}`}>
                            <button className={`w-10 h-10 flex items-center justify-center ${textColor} hover:opacity-80 transition-opacity`}>
                        ...
                        <div className="flex items-start gap-6">
                            <div className="flex-1">
                                ...
                                <div className="flex gap-8 mt-6">
                                    <button className="text-xs font-bold tracking-[0.15em] uppercase border-b-2 border-[#181728] pb-2 text-[#181728]">
                                        BUY
                                    </button>
                                    <button className="text-xs font-bold tracking-[0.15em] uppercase text-gray-400 hover:text-[#181728] pb-2 border-b-2 border-transparent hover:border-gray-300 transition-all">
                                        RENT
                                    </button>
                                    <button className="text-xs font-bold tracking-[0.15em] uppercase text-gray-400 hover:text-[#181728] pb-2 border-b-2 border-transparent hover:border-gray-300 transition-all">
                                        AGENTS
